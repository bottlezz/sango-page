...
(async()=>{
 ...
 assert.deepEqual(result.metrics.transactions,[]);assert.equal(result.metrics.updates.length,1);assert.equal(result.metrics.updates[0].paths.includes('game/6'),false);
 await browser.close();console.log('PASS: B05 confirms two selected generals with two reads, no locks, and one narrow update.');
