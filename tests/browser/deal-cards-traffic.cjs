...
(async()=>{
 ...
 const result=await page.evaluate(async()=>{
   ...
   const metrics=f.getMetrics(),hands=Array.from({length:6},(_,i)=>Object.values(f.read(`game/6/p${i+1}/hand/cards`)).map(card=>card.id));
 ...
 assert.deepEqual(result.metrics.gets.map(item=>item.path).sort(),[
   'game/6/tableDecks/pai/cards',...Array.from({length:6},(_,i)=>`game/6/p${i+1}/hand/cards`)
 ].sort());
 assert.equal(result.metrics.updates.length,1);assert.equal(result.metrics.updates[0].paths.includes('game/6'),false);
 await browser.close();console.log('PASS: B02 deals four cards with narrow reads, one update and only the draw-pile lock.');
