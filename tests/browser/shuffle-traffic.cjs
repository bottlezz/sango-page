...
(async()=>{
 ...
 const result=await page.evaluate(async()=>{const f=window.fixture;f.resetMetrics();await f.controller.resetPai();return {metrics:f.getMetrics(),pai:f.read('game/6/tableDecks/pai/cards'),bottom:f.read('game/6/tableDecks/paiBottom'),discard:f.read('game/6/tableDecks/discard')};});
 assert.equal(Object.keys(result.pai).length,200);assert.equal(result.bottom,null);assert.equal(result.discard,null);
 assert.ok(Object.values(result.pai).every(card=>card.show==='0'&&card.judgmentEffect===undefined&&card.panOrder===undefined));
 assert.deepEqual(result.metrics.gets.map(item=>item.path).sort(),['game/6/tableDecks/pai/cards','game/6/tableDecks/paiBottom/cards','game/6/tableDecks/discard/cards'].sort());
 assert.ok(result.metrics.transactions.every(item=>item.path.startsWith('game/6/operationLocks/')));
